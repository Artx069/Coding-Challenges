...
function calculateWinners(snapshot, penguins) {
  let snapshotSplit = snapshot.split("\n");
  let arr = [];
  for (let i = 0; i < snapshotSplit.length; i++) {
    let toGo = snapshotSplit[i].replace(/^.*[pP]/, "").replace("|", "");
    let toGoNum =
      toGo.split("").filter((a) => a == "-").length +
      toGo.split("").filter((a) => a == "~").length * 2;
    arr.push([toGoNum, penguins[i]]);
  }
  arr = arr.sort((a, b) => a[0] - b[0]);

  return `GOLD: ${arr[0][1]}, SILVER: ${arr[1][1]}, BRONZE: ${arr[2][1]}`;
}
