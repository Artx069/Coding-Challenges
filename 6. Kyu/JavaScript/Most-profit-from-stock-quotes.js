...
function getMostProfitFromStockQuotes(quotes) {
  ...
  while (quotes.length > 0) {
    ...
    for (let num in quotes) {
      if (quotes[num] > highest) {
        highest = quotes[num];
      }
    }

    for (let i = 0; i < quotes.indexOf(highest); i++) {
      sum += highest - quotes[i];
    }

    quotes = quotes.slice(quotes.indexOf(highest) + 1, quotes.length);
  }
  return sum;
}
