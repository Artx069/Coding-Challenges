...
function splitWorkload(workload) {
  ...
  while (bob.length != 0) {
    let scoreBob = bob.length > 0 ? bob.reduce((a, b) => a + b) : 0;
    let scoreJim = jim.length > 0 ? jim.reduce((a, b) => a + b) : 0;
    diff.push(Math.abs(scoreBob - scoreJim));
    counter++;
    jim.push(bob.shift());
  }
  let min = Math.min(...diff);
  return [diff.indexOf(min), min];
}
