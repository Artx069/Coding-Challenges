...
function sortCsvColumns(csvFileContent) {
  let nSplittedByNewLine = csvFileContent.split("\n");

  let sol = [];

  for (let content of nSplittedByNewLine[0].split(";")) {
    let temp = [];
    temp.push(content);
    sol.push(temp);
  }
  nSplittedByNewLine.shift();

  for (let line of nSplittedByNewLine) {
    let temp = line.split(";");
    for (let i = 0; i < temp.length; i++) {
      sol[i].push(temp[i]);
    }
  }
  sol = sol.sort(function (a, b) {
