...
let hypertensive = function (patients) {
  patients = patients
    .map((x) => x.map((y) => y.split("/").map((patient) => parseInt(patient))))
    .filter((x) => {
      if (x.filter((y) => y[0] >= 180 && y[1] >= 110).length > 0) {
        return x;
      }
      if (
        x.length > 1 &&
        x.map((y) => y[0]).reduce((a, b) => a + b) / x.length >= 140
      ) {
        return x;
      }
      if (
        x.length > 1 &&
        x.map((y) => y[1]).reduce((a, b) => a + b) / x.length >= 90
      ) {
        return x;
      }
    });
