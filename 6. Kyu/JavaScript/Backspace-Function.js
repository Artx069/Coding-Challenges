...
function solve(s) {
  s = s.replace(
    ...
    function (match, contents, offset, input_string) {
      return "[backspace]".repeat(contents);
    }
  );

  while (s.match(/\[backspace\]/)) {
    let index = s.indexOf("[backspace]") == 0 ? 0 : s.indexOf("[backspace]");
    if (index == 0) {
      s = s.replace("[backspace]", "");
    } else {
      s = s.substring(0, index - 1) + s.substring(index + 11);
    }
  }
  return s;
}
