...
function getWinner(listOfBallots) {
  let representatives = new Set(listOfBallots);
  let highest = [0, 0];
  for (let key of representatives) {
    let votes = listOfBallots.filter((x) => x == key).length;
    if (votes > highest[1]) {
      highest = [key, votes];
    }
  }
  return highest[1] > Math.floor(listOfBallots.length / 2) ? highest[0] : null;
}
