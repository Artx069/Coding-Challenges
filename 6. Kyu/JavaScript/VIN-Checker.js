...
const checkVin = (vin) => {
  ...
  const conv = "12345678012345070923456789";
  const weigths = [8, 7, 6, 5, 4, 3, 2, 10, 0, 9, 8, 7, 6, 5, 4, 3, 2];

  if (vin.match(/[^A-HJ-NPR-Z0-9]/) || vin.length != 17) {
    return false;
  }
  ...
  let vinConv = vin
    ...
    .map((x, i, arr) =>
      x.match(/[0-9]/)
        ? Number(x) * weigths[i]
        : Number(conv.charAt(alph.indexOf(x))) * weigths[i]
    );
  let vinSum = vinConv.reduce((a, b) => a + b);
  let vinMod = vinSum % 11 == 10 ? "X" : vinSum % 11;

  return vin.charAt(8) == vinMod;
};
