...
function checkAvailability(schedule, currentTime) {
  let check = schedule.filter(
    (x) =>
      parseInt(x[0].replace(/:/g, "")) <
        parseInt(currentTime.replace(/:/g, "")) &&
      parseInt(x[1].replace(/:/g, "")) > parseInt(currentTime.replace(/:/g, ""))
  );
  return check.length <= 0 ? true : check[0][1];
}
