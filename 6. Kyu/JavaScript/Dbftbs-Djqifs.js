...
function encryptor(key, message) {
  ...
  let ALPHABET_UPP = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  key = key % 26;
  return message
    .split("")
    .map((x) =>
      x.match(/[a-z]/)
        ? ALPHABET_LOW.charAt(
            ALPHABET_LOW.indexOf(x) + key < 0
              ? ALPHABET_LOW.indexOf(x) + key + 26
              : (ALPHABET_LOW.indexOf(x) + key) % 26
          )
        : x.match(/[A-Z]/)
        ? ALPHABET_UPP.charAt(
            ALPHABET_UPP.indexOf(x) + key < 0
              ? ALPHABET_UPP.indexOf(x) + key + 26
              : (ALPHABET_UPP.indexOf(x) + key) % 26
          )
        : x
    )
