...
*/
public class TheCurse {

    static String translate(String speech, String[] vocabulary) {
        speech = speech.replaceAll("[*]", "@");

        String[] wordsOfSpeech = speech.replaceAll("[^a-zA-Z @]+", "").split(" ");
        for(String word : wordsOfSpeech){
            String reg = word.replaceAll("[@]", ".");
            for(String voc : vocabulary){
                if(voc.matches("^" + reg + "$")){
                    speech = speech.replaceFirst(word, voc);
                    break;
                }
            }
        }
