...
public class FindaChair {
    public static Object meeting(Room[] x, int need) {
        ...
        for(int i = 0; i < x.length; i++){
          int availableChairs = x[i].chairs - x[i].occupants.length() < 0 ? 0 : x[i].chairs - x[i].occupants.length();
          
          if(availableChairs >= need){
            solution.add(need);
            need -= need;
          }else{
            solution.add(availableChairs);
        ...
        return need > 0 ? "Not enough!" : solution.stream().mapToInt(i -> i).toArray();
      }
}
