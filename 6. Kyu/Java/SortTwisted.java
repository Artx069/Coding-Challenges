...
*/
public class SortTwisted {
    public static Integer[] sortTwisted37(Integer[] array) {
        Integer[] result = Arrays.copyOf(Arrays.stream(array).map(x -> {
            String forReturn = "";
            for (String c : x.toString().split("")) {
                if (c.equals("3")) {
                    forReturn += "7";
                } else if (c.equals("7")) {
                    forReturn += "3";
            ...
            Integer parsed = Integer.parseInt(forReturn);
            return parsed;
        }).sorted().map(x -> {
            String forReturn = "";
            for (String c : x.toString().split("")) {
            ...
            Integer parsed = Integer.parseInt(forReturn);
            return parsed;
        }).toArray(), array.length, Integer[].class);
        return result;
    }
}
