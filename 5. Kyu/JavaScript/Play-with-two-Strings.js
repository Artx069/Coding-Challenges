...
function workOnStrings(a, b) {
  ...
  for (let char of a) {
    for (let j = 0; j < b.length; j++) {
      if (char.toLowerCase() == b[j].toLowerCase()) {
        b[j] == b[j].toLowerCase()
          ? (b[j] = char.toUpperCase())
          : (b[j] = char.toLowerCase());
      }
    }
  }
  ...
  for (let char of b) {
    for (let j = 0; j < a.length; j++) {
      if (char.toLowerCase() == a[j].toLowerCase()) {
        a[j] == a[j].toLowerCase()
          ? (a[j] = char.toUpperCase())
          : (a[j] = char.toLowerCase());
      }
    }
  }
