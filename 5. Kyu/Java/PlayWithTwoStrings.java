    ...
    public static String workOnStrings(String a, String b) {
        ...
        for(String s : aSplit){
            for(int j = 0; j < bSplit.length; j++){
                if(s.toLowerCase().equals(bSplit[j].toLowerCase())){
                    if (bSplit[j].equals(bSplit[j].toLowerCase())) {
                        bSplit[j] = s.toUpperCase();
                    } else {
                        bSplit[j] = s.toLowerCase();
                    }
                }
            }
        }
        ...
        for(String s : bSplit){
            for(int j = 0; j < aSplit.length; j++){
                if(s.toLowerCase().equals(aSplit[j].toLowerCase())){
                    if (aSplit[j].equals(aSplit[j].toLowerCase())) {
                        aSplit[j] = s.toUpperCase();
                    } else {
                        aSplit[j] = s.toLowerCase();
                    }
                }
            }
        }
        return String.join("", aSplit) + String.join("", bSplit);
      }
}
